...
<?php 
$user_id = $_GET['uid'];
$query = pg_query("select * from rayon where id_rayon='$user_id'");
...
<body>
  <div class="container">
    <section class="register">
      <h1>Tambah Rayon</h1>
      <form method="post" action="update_rayon.php">
      <div>
      <h3>ID Rayon</h3>
      <input type="text" name="id" value="<?php echo $data['id_rayon']; ?>" required>
      <h3>Nama Rayon</h3>
      <input type="text" name="nama" value="<?php echo $data['nama_rayon'];?>" required>
      <h3>Alamat Rayon</h3>
      <input type="text" name="kec" value="<?php echo $data['kecamatan'];?>" required>
      <h3>ID Wilayah</h3>
